...
def human_size_path(path: str) -> str:
    """Formatta la dimensione di un file dato il path (non i byte grezzi)."""
    try:
    ...
    def writer_progress(total, current, elapsed, label):
        ...
        if label == "scrittura":
            ...
            msg = (
                ...
                f"({human_size_bytes(current)} di {human_size_bytes(total)}, {elapsed} s). "
            ...
            msg = (
                ...
                f"({human_size_bytes(current)} / {human_size_bytes(total)})."
            )
        else:
