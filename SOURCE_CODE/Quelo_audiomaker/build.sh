...
LB_DIR="/var/tmp/quelo-audiomaker-live-build"
LB_CACHE="/var/tmp/quelo-audiomaker-lb-cache"
MSFONT_CACHE="/var/tmp/quelo-audiomaker-mscorefonts-cache"
...
OUTPUT_ISO="${PROJECT}/ISO/Quelo-Audiomaker-${VERSION}-alpha.iso"
SRC_BACKUP="${PROJECT}/SOURCE_CODE/_backup_Quelo_audiomaker"
...
# Unico backup dei sorgenti precedenti (sostituito a ogni build).
# Serve solo per un passo indietro; non tocca la cartella ISO.
quelo_backup_source() {
  local tmp="${SRC_BACKUP}.tmp"
  echo "Backup SOURCE_CODE precedente → ${SRC_BACKUP}"
  rm -rf "${tmp}"
  mkdir -p "${tmp}"
  if command -v rsync >/dev/null 2>&1; then
    rsync -a --delete "${SORGENTI}/" "${tmp}/"
  else
    cp -a "${SORGENTI}/." "${tmp}/"
  fi
  rm -rf "${SRC_BACKUP}"
  mv "${tmp}" "${SRC_BACKUP}"
  chown -R "${BUILD_USER}:${BUILD_USER}" "${SRC_BACKUP}" 2>/dev/null || true
}
...
lb config \
  ...
  --iso-application "Quelo Audiomaker ${VERSION} alpha" \
  --iso-volume "QUELO-AUDIOMAKER-${VERSION}" \
  --memtest none \
  --win32-loader false \
...
echo "${VERSION}" >"${LB_DIR}/config/includes.chroot/etc/quelo-audiomaker-version"
...
# Pool ISO (non squashfs): grub-pc per install BIOS legacy offline.
# Confligge con grub-efi già nel chroot → solo *.list.binary → pool/dists.
if [[ -f "${SORGENTI}/packages/grub-bios.list.binary" ]]; then
  cp -a "${SORGENTI}/packages/grub-bios.list.binary" \
    "${LB_DIR}/config/package-lists/grub-bios.list.binary"
  echo "Pool ISO: incluso grub-bios.list.binary (grub-pc offline)"
fi
...
# Tutte le release restano in ISO/ (nessuna cancellazione automatica).
mkdir -p "${PROJECT}/ISO"
cp -f "${ISO}" "${OUTPUT_ISO}"
...
echo "Backup sorgenti: ${SRC_BACKUP}/ (unico, sostituito a ogni build)"
