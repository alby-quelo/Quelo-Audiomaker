...
#
# ID voce utente (--only): network cups bluetooth config audio display firefox audacity audacious mixxx ardour reaper butt ffado powermanagement
set -o pipefail
...
load_audacity() {
  session_debug "=== LOAD audacity ==="
  local src="${STORE}/audacity"
  local archive="${src}/audacity-config.tar.gz"
  session_debug_path "store-audacity" "${src}" 3

  if [[ -f "${archive}" ]]; then
    if session_restore_audacity_config "${archive}"; then
      step_ok "configurazione Audacity ripristinata"
      verbose "   Riavvia Audacity."
      return 0
    fi
    step_err "audacity (estrazione tar fallita)"
    return 1
  fi

  step_skip "audacity (audacity-config.tar.gz assente in store — salva di nuovo)"
  return 2
}

load_audacious() {
  session_debug "=== LOAD audacious ==="
  local src="${STORE}/audacious"
  local archive="${src}/audacious-config.tar.gz"
  session_debug_path "store-audacious" "${src}" 3

  if [[ -f "${archive}" ]]; then
    if session_restore_audacious_config "${archive}"; then
      step_ok "configurazione Audacious ripristinata (~/.config/audacious)"
      verbose "   Riavvia Audacious."
      return 0
    fi
    step_err "audacious (estrazione tar fallita)"
    return 1
  fi

  step_skip "audacious (audacious-config.tar.gz assente in store — salva di nuovo)"
  return 2
}

load_mixxx() {
  session_debug "=== LOAD mixxx ==="
  local src="${STORE}/mixxx"
  local archive="${src}/mixxx-config.tar.gz"
  session_debug_path "store-mixxx" "${src}" 3

  if [[ -f "${archive}" ]]; then
    if session_restore_mixxx_config "${archive}"; then
      step_ok "configurazione Mixxx ripristinata (~/.mixxx)"
      verbose "   Riavvia Mixxx."
      return 0
    fi
    step_err "mixxx (estrazione tar fallita)"
    return 1
  fi

  step_skip "mixxx (mixxx-config.tar.gz assente in store — salva di nuovo)"
  return 2
}

load_ardour() {
  session_debug "=== LOAD ardour ==="
  local src="${STORE}/ardour"
  local archive="${src}/ardour-config.tar.gz"
  session_debug_path "store-ardour" "${src}" 3

  if [[ -f "${archive}" ]]; then
    if session_restore_ardour_config "${archive}"; then
      step_ok "configurazione Ardour ripristinata (~/.config/ardour*)"
      verbose "   Riavvia Ardour."
      return 0
    fi
    step_err "ardour (estrazione tar fallita)"
    return 1
  fi

  step_skip "ardour (ardour-config.tar.gz assente in store — salva di nuovo)"
  return 2
}

load_reaper() {
  session_debug "=== LOAD reaper ==="
  local src="${STORE}/reaper"
  local archive="${src}/reaper-config.tar.gz"
  session_debug_path "store-reaper" "${src}" 3

  if [[ -f "${archive}" ]]; then
    if session_restore_reaper_config "${archive}"; then
      step_ok "configurazione REAPER ripristinata (~/.config/REAPER)"
      verbose "   Riavvia REAPER."
      return 0
    fi
    step_err "reaper (estrazione tar fallita)"
    return 1
  fi

  step_skip "reaper (reaper-config.tar.gz assente in store — salva di nuovo)"
  return 2
}

load_butt() {
  session_debug "=== LOAD butt ==="
  local src="${STORE}/butt"
  local archive="${src}/butt-config.tar.gz"
  session_debug_path "store-butt" "${src}" 3

  if [[ -f "${archive}" ]]; then
    if session_restore_butt_config "${archive}"; then
      step_ok "configurazione BUTT ripristinata (~/.buttrc)"
      verbose "   Riavvia STREAMING LIVE (BUTT) se aperto."
      return 0
    fi
    step_err "butt (estrazione tar fallita)"
    return 1
  fi

  step_skip "butt (butt-config.tar.gz assente in store — salva di nuovo)"
  return 2
}

load_ffado() {
  session_debug "=== LOAD ffado ==="
  local src="${STORE}/ffado"
  local archive="${src}/ffado-config.tar.gz"
  session_debug_path "store-ffado" "${src}" 3

  if [[ -f "${archive}" ]]; then
    if session_restore_ffado_config "${archive}"; then
      step_ok "configurazione FireWire FFADO ripristinata (~/.ffado)"
      verbose "   Riavvia ffado-mixer se usato."
      return 0
    fi
    step_err "ffado (estrazione tar fallita)"
    return 1
  fi

  step_skip "ffado (ffado-config.tar.gz assente in store — salva di nuovo)"
  return 2
}

load_powermanagement() {
  session_debug "=== LOAD powermanagement ==="
  local src="${STORE}/powermanagement"
  local archive="${src}/powermanagement-config.tar.gz"
  session_debug_path "store-powermanagement" "${src}" 3

  if [[ -f "${archive}" ]]; then
    if session_restore_powermanagement_config "${archive}"; then
      step_ok "gestione energia ripristinata (lxqt-powermanagement.conf)"
      verbose "   Riavvia lxqt-powermanagement o la sessione per vedere le icone aggiornate."
      return 0
    fi
    step_err "powermanagement (estrazione tar fallita)"
    return 1
  fi

  step_skip "powermanagement (powermanagement-config.tar.gz assente in store — salva di nuovo)"
  return 2
}
...
if [[ "${LOAD_GUI}" -eq 0 ]]; then
  echo ""
  echo "Quelo Audiomaker — load_sessione ${QUELO_SESSION_VERSION}"
  echo "======================================================="
fi
...
if [[ ${#LOAD_STEPS[@]} -gt 0 ]]; then
  ...
  run_batch_step audacity "Configurazione Audacity" load_audacity || true
  run_batch_step audacious "Configurazione Audacious" load_audacious || true
  run_batch_step mixxx "Configurazione Mixxx" load_mixxx || true
  run_batch_step ardour "Configurazione Ardour" load_ardour || true
  run_batch_step reaper "Configurazione REAPER" load_reaper || true
  run_batch_step butt "Configurazione BUTT (streaming)" load_butt || true
  run_batch_step ffado "Configurazione FireWire (FFADO)" load_ffado || true
  run_batch_step powermanagement "Gestione energia (batteria)" load_powermanagement || true
else
  run_step "1/18" "Hostname" load_hostname
  run_step "2/18" "Machine-ID" load_machine_id
  run_step "3/18" "Rete (NetworkManager)" load_network
  run_step "4/18" "Stampanti (CUPS)" load_cups
  run_step "5/18" "Bluetooth" load_bluetooth
  run_step "6/18" "Config (~/.config)" load_config
  run_step "7/18" "Audio" load_audio
  run_step "8/18" "Schermo (xrandr)" load_display
  run_step "9/18" "Firefox" load_firefox
  run_step "10/18" "Configurazione Audacity" load_audacity
  run_step "11/18" "Configurazione Audacious" load_audacious
  run_step "12/18" "Configurazione Mixxx" load_mixxx
  run_step "13/18" "Configurazione Ardour" load_ardour
  run_step "14/18" "Configurazione REAPER" load_reaper
  run_step "15/18" "Configurazione BUTT (streaming)" load_butt
  run_step "16/18" "Configurazione FireWire (FFADO)" load_ffado
  run_step "17/18" "Gestione energia (batteria)" load_powermanagement
  run_step "18/18" "Sorgenti apt" load_apt_sources
fi
