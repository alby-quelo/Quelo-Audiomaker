...
SRC="/usr/share/applications/quelo-install.desktop"
LOG_TAG="quelo-desktop-install-icon"
...
place() {
  ...
  dest="${desktop_dir}/quelo-install.desktop"
...
for _ in $(seq 1 24); do
  ...
  if [[ -n "${ddir}" && -f "${ddir}/quelo-install.desktop" && "${ddir}" == */media/quelo-home/* ]]; then
    sleep 3
    place || true
    sleep 5
    place || true
