...
# Salva/ripristino sessione: SOLO ambiente live (persistence USB).
quelo_is_live_boot() {
  [[ -d /run/live ]] && return 0
  [[ -d /lib/live/mount/rootfs ]] && return 0
...
# --- Audacity: preferenze + dati (NO AutoSave/cache pesanti) ---
# Path tipici: ~/.config/audacity/  ~/.local/share/audacity/  ~/.audacity-data/ (legacy)

audacity_quit_if_running() {
  local waited=0
  pgrep -x audacity >/dev/null 2>&1 || return 0
  session_debug "Chiusura Audacity (necessaria prima del backup/restore)..."
  pkill -TERM -x audacity 2>/dev/null || true
  while pgrep -x audacity >/dev/null 2>&1 && [[ "${waited}" -lt 20 ]]; do
    sleep 1
    waited=$((waited + 1))
  done
  if pgrep -x audacity >/dev/null 2>&1; then
    pkill -KILL -x audacity 2>/dev/null || true
    sleep 1
  fi
}

session_backup_audacity_config() {
  local archive="$1"
  local have=0
  local -a paths=()

  session_debug "=== BACKUP AUDACITY -> ${archive} ==="
  audacity_quit_if_running
  session_debug_cmd "mkdir-archive" mkdir -p "$(dirname "${archive}")"

  if [[ -d /root/.config/audacity ]]; then
    paths+=(.config/audacity)
    have=1
  fi
  if [[ -d /root/.local/share/audacity ]]; then
    paths+=(.local/share/audacity)
    have=1
  fi
  if [[ -d /root/.audacity-data ]]; then
    paths+=(.audacity-data)
    have=1
  fi
  [[ "${have}" -eq 1 ]] || return 1

  session_debug_cmd "tar-backup-audacity" tar -czf "${archive}" -C /root \
    --exclude='.config/audacity/AutoSave' \
    --exclude='.local/share/audacity/AutoSave' \
    --exclude='.audacity-data/AutoSave' \
    --exclude='.local/share/audacity/Cache' \
    --exclude='.audacity-data/Cache' \
    "${paths[@]}"
  [[ -f "${archive}" && -s "${archive}" ]]
}

session_restore_audacity_config() {
  local archive="$1"

  session_debug "=== RESTORE AUDACITY ${archive} ==="
  [[ -f "${archive}" && -s "${archive}" ]] || return 1

  audacity_quit_if_running
  if [[ -d /root/.config/audacity ]]; then
    session_debug_cmd "mv-old-audacity-config" mv /root/.config/audacity "/root/.config/audacity.quelo-bak.$(date +%s)" 2>/dev/null || true
  fi
  if [[ -d /root/.local/share/audacity ]]; then
    session_debug_cmd "mv-old-audacity-share" mv /root/.local/share/audacity "/root/.local/share/audacity.quelo-bak.$(date +%s)" 2>/dev/null || true
  fi
  if [[ -d /root/.audacity-data ]]; then
    session_debug_cmd "mv-old-audacity-data" mv /root/.audacity-data "/root/.audacity-data.quelo-bak.$(date +%s)" 2>/dev/null || true
  fi
  session_debug_cmd "mkdir-audacity-parents" mkdir -p /root/.config /root/.local/share
  session_debug_cmd "tar-restore-audacity" tar -xzf "${archive}" -C /root
  [[ -d /root/.config/audacity || -d /root/.local/share/audacity || -d /root/.audacity-data ]]
}

# --- Audacious: preferenze / playlist (~/.config/audacious/) ---

audacious_quit_if_running() {
  local waited=0
  pgrep -x audacious >/dev/null 2>&1 || return 0
  session_debug "Chiusura Audacious (necessaria prima del backup/restore)..."
  pkill -TERM -x audacious 2>/dev/null || true
  while pgrep -x audacious >/dev/null 2>&1 && [[ "${waited}" -lt 20 ]]; do
    sleep 1
    waited=$((waited + 1))
  done
  if pgrep -x audacious >/dev/null 2>&1; then
    pkill -KILL -x audacious 2>/dev/null || true
    sleep 1
  fi
}

session_backup_audacious_config() {
  local archive="$1"
  local share=".config/audacious"
  local root="/root/${share}"

  session_debug "=== BACKUP AUDACIOUS ${root} -> ${archive} ==="
  [[ -d "${root}" ]] || return 1

  audacious_quit_if_running
  session_debug_cmd "mkdir-archive" mkdir -p "$(dirname "${archive}")"
  session_debug_cmd "tar-backup-audacious" tar -czf "${archive}" -C /root \
    --exclude="${share}/cache" \
    --exclude="${share}/Cache" \
    "${share}"
  [[ -f "${archive}" && -s "${archive}" ]]
...
session_restore_audacious_config() {
  local archive="$1"
  local root="/root/.config/audacious"

  session_debug "=== RESTORE AUDACIOUS ${archive} -> ${root} ==="
  [[ -f "${archive}" && -s "${archive}" ]] || return 1

  audacious_quit_if_running
  if [[ -d "${root}" ]]; then
    session_debug_cmd "mv-old-audacious" mv "${root}" "${root}.quelo-bak.$(date +%s)" 2>/dev/null || true
  fi
  session_debug_cmd "mkdir-audacious-parent" mkdir -p /root/.config
  session_debug_cmd "tar-restore-audacious" tar -xzf "${archive}" -C /root
  [[ -d "${root}" ]]
}

# --- Mixxx: ~/.mixxx/ (preferenze, controller utente, libreria DB) ---
# Esclude analysis/ (waveform rigenerabili) e log.

mixxx_quit_if_running() {
  local waited=0
  pgrep -x mixxx >/dev/null 2>&1 || return 0
  session_debug "Chiusura Mixxx (necessaria prima del backup/restore)..."
  pkill -TERM -x mixxx 2>/dev/null || true
  while pgrep -x mixxx >/dev/null 2>&1 && [[ "${waited}" -lt 20 ]]; do
    sleep 1
    waited=$((waited + 1))
  done
  if pgrep -x mixxx >/dev/null 2>&1; then
    pkill -KILL -x mixxx 2>/dev/null || true
    sleep 1
  fi
}

session_backup_mixxx_config() {
  local archive="$1"
  local share=".mixxx"
  local root="/root/${share}"

  session_debug "=== BACKUP MIXXX ${root} -> ${archive} ==="
  [[ -d "${root}" ]] || return 1

  mixxx_quit_if_running
  session_debug_cmd "mkdir-archive" mkdir -p "$(dirname "${archive}")"
  session_debug_cmd "tar-backup-mixxx" tar -czf "${archive}" -C /root \
    --exclude="${share}/analysis" \
    --exclude="${share}/Analysis" \
    --exclude="${share}/mixxx.log" \
    --exclude="${share}/mixxx.log.*" \
    --exclude="${share}/*.log" \
    "${share}"
  [[ -f "${archive}" && -s "${archive}" ]]
...
session_restore_mixxx_config() {
  local archive="$1"
  local root="/root/.mixxx"

  session_debug "=== RESTORE MIXXX ${archive} -> ${root} ==="
  [[ -f "${archive}" && -s "${archive}" ]] || return 1

  mixxx_quit_if_running
  if [[ -d "${root}" ]]; then
    session_debug_cmd "mv-old-mixxx" mv "${root}" "${root}.quelo-bak.$(date +%s)" 2>/dev/null || true
  fi
  session_debug_cmd "tar-restore-mixxx" tar -xzf "${archive}" -C /root
  [[ -d "${root}" ]]
}

# --- Ardour: ~/.config/ardour* e ~/.local/share/ardour* ---

ardour_quit_if_running() {
  local waited=0
  pgrep -x ardour >/dev/null 2>&1 \
    || pgrep -x ardour8 >/dev/null 2>&1 \
    || pgrep -x ardour7 >/dev/null 2>&1 \
    || pgrep -x Ardour >/dev/null 2>&1 \
    || return 0
  session_debug "Chiusura Ardour (necessaria prima del backup/restore)..."
  pkill -TERM -x ardour 2>/dev/null || true
  pkill -TERM -x ardour8 2>/dev/null || true
  pkill -TERM -x ardour7 2>/dev/null || true
  pkill -TERM -x Ardour 2>/dev/null || true
  while (pgrep -x ardour >/dev/null 2>&1 \
      || pgrep -x ardour8 >/dev/null 2>&1 \
      || pgrep -x ardour7 >/dev/null 2>&1) \
      && [[ "${waited}" -lt 20 ]]; do
    sleep 1
  ...
  if pgrep -x ardour >/dev/null 2>&1 || pgrep -x ardour8 >/dev/null 2>&1; then
    pkill -KILL -x ardour 2>/dev/null || true
    pkill -KILL -x ardour8 2>/dev/null || true
    sleep 1
  fi
}

session_ardour_paths() {
  local p
  shopt -s nullglob
  for p in /root/.config/ardour* /root/.local/share/ardour*; do
    [[ -e "${p}" ]] || continue
    printf '%s\n' "${p#/root/}"
  done
  shopt -u nullglob
...
session_backup_ardour_config() {
  local archive="$1"
  local -a paths=()
  local line

  session_debug "=== BACKUP ARDOUR -> ${archive} ==="
  while IFS= read -r line; do
    [[ -n "${line}" ]] && paths+=("${line}")
  done < <(session_ardour_paths)
  [[ ${#paths[@]} -gt 0 ]] || return 1

  ardour_quit_if_running
  session_debug_cmd "mkdir-archive" mkdir -p "$(dirname "${archive}")"
  # Esclude cache/analisies rigenerabili
  session_debug_cmd "tar-backup-ardour" tar -czf "${archive}" -C /root \
    --exclude='.config/ardour*/cache' \
    --exclude='.config/ardour*/Cache' \
    --exclude='.local/share/ardour*/cache' \
    --exclude='.local/share/ardour*/Cache' \
    --exclude='.local/share/ardour*/instant.xml' \
    "${paths[@]}"
  [[ -f "${archive}" && -s "${archive}" ]]
}

session_restore_ardour_config() {
  local archive="$1"
  local p

  session_debug "=== RESTORE ARDOUR ${archive} ==="
  [[ -f "${archive}" && -s "${archive}" ]] || return 1
  ...
  for p in /root/.config/ardour* /root/.local/share/ardour*; do
    [[ -e "${p}" ]] || continue
    session_debug_cmd "mv-old-ardour" mv "${p}" "${p}.quelo-bak.$(date +%s)" 2>/dev/null || true
  done
  shopt -u nullglob
  session_debug_cmd "mkdir-ardour-parents" mkdir -p /root/.config /root/.local/share
  session_debug_cmd "tar-restore-ardour" tar -xzf "${archive}" -C /root
  [[ -n "$(session_ardour_paths | head -1)" ]]
}

# --- REAPER: ~/.config/REAPER ---

reaper_quit_if_running() {
  local waited=0
  pgrep -x reaper >/dev/null 2>&1 || return 0
  session_debug "Chiusura REAPER (necessaria prima del backup/restore)..."
  pkill -TERM -x reaper 2>/dev/null || true
  while pgrep -x reaper >/dev/null 2>&1 && [[ "${waited}" -lt 20 ]]; do
    sleep 1
    waited=$((waited + 1))
  done
...
session_backup_reaper_config() {
  local archive="$1"
  local share=".config/REAPER"
  local root="/root/${share}"

  session_debug "=== BACKUP REAPER ${root} -> ${archive} ==="
  [[ -d "${root}" ]] || return 1

  reaper_quit_if_running
  session_debug_cmd "mkdir-archive" mkdir -p "$(dirname "${archive}")"
  session_debug_cmd "tar-backup-reaper" tar -czf "${archive}" -C /root \
    --exclude="${share}/Cache" \
    --exclude="${share}/cache" \
    --exclude="${share}/*.log" \
    --exclude="${share}/logs" \
...
session_restore_reaper_config() {
  local archive="$1"
  local root="/root/.config/REAPER"

  session_debug "=== RESTORE REAPER ${archive} -> ${root} ==="
  [[ -f "${archive}" && -s "${archive}" ]] || return 1

  reaper_quit_if_running
  if [[ -d "${root}" ]]; then
    session_debug_cmd "mv-old-reaper" mv "${root}" "${root}.quelo-bak.$(date +%s)" 2>/dev/null || true
  fi
  session_debug_cmd "mkdir-reaper-parent" mkdir -p /root/.config
  session_debug_cmd "tar-restore-reaper" tar -xzf "${archive}" -C /root
  [[ -d "${root}" ]]
}

# --- BUTT (streaming): ~/.buttrc ---

session_backup_butt_config() {
  local archive="$1"
  local conf=".buttrc"
  local conf_abs="/root/${conf}"

  session_debug "=== BACKUP BUTT ${conf_abs} -> ${archive} ==="
  [[ -f "${conf_abs}" ]] || return 1

  session_debug_cmd "mkdir-archive" mkdir -p "$(dirname "${archive}")"
  session_debug_cmd "tar-backup-butt" tar -czf "${archive}" -C /root "${conf}"
  [[ -f "${archive}" && -s "${archive}" ]]
}

session_restore_butt_config() {
  local archive="$1"
  local conf_abs="/root/.buttrc"

  session_debug "=== RESTORE BUTT ${archive} -> ${conf_abs} ==="
  [[ -f "${archive}" && -s "${archive}" ]] || return 1

  if [[ -f "${conf_abs}" ]]; then
    session_debug_cmd "mv-old-butt" mv "${conf_abs}" "${conf_abs}.quelo-bak.$(date +%s)" 2>/dev/null || true
  fi
  session_debug_cmd "tar-restore-butt" tar -xzf "${archive}" -C /root
  [[ -f "${conf_abs}" ]]
}

# --- FFADO (FireWire audio): ~/.ffado/ ---

session_backup_ffado_config() {
  local archive="$1"
  local share=".ffado"
  local root="/root/${share}"

  session_debug "=== BACKUP FFADO ${root} -> ${archive} ==="
  [[ -d "${root}" ]] || return 1

  session_debug_cmd "mkdir-archive" mkdir -p "$(dirname "${archive}")"
  session_debug_cmd "tar-backup-ffado" tar -czf "${archive}" -C /root \
    --exclude="${share}/*.log" \
    --exclude="${share}/logs" \
    "${share}"
  [[ -f "${archive}" && -s "${archive}" ]]
...
session_restore_ffado_config() {
  local archive="$1"
  local root="/root/.ffado"

  session_debug "=== RESTORE FFADO ${archive} -> ${root} ==="
  [[ -f "${archive}" && -s "${archive}" ]] || return 1

  if [[ -d "${root}" ]]; then
    session_debug_cmd "mv-old-ffado" mv "${root}" "${root}.quelo-bak.$(date +%s)" 2>/dev/null || true
  fi
  session_debug_cmd "tar-restore-ffado" tar -xzf "${archive}" -C /root
  [[ -d "${root}" ]]
}

# --- LXQt Power Management: icona batteria / preferenze energia ---
# Path: ~/.config/lxqt/lxqt-powermanagement.conf
# (anche incluso in "Desktop" via mirror ~/.config; voce dedicata per salvataggio selettivo)

session_backup_powermanagement_config() {
  local archive="$1"
  local conf=".config/lxqt/lxqt-powermanagement.conf"
  local conf_abs="/root/${conf}"

  session_debug "=== BACKUP POWERMANAGEMENT ${conf_abs} -> ${archive} ==="
  [[ -f "${conf_abs}" ]] || return 1

  session_debug_cmd "mkdir-archive" mkdir -p "$(dirname "${archive}")"
  session_debug_cmd "tar-backup-powermanagement" tar -czf "${archive}" -C /root "${conf}"
  [[ -f "${archive}" && -s "${archive}" ]]
}

session_restore_powermanagement_config() {
  local archive="$1"
  local conf_abs="/root/.config/lxqt/lxqt-powermanagement.conf"
  local conf_dir="/root/.config/lxqt"

  session_debug "=== RESTORE POWERMANAGEMENT ${archive} -> ${conf_abs} ==="
  [[ -f "${archive}" && -s "${archive}" ]] || return 1

  session_debug_cmd "mkdir-lxqt" mkdir -p "${conf_dir}"
  if [[ -f "${conf_abs}" ]]; then
    session_debug_cmd "mv-old-powermanagement" mv "${conf_abs}" "${conf_abs}.quelo-bak.$(date +%s)" 2>/dev/null || true
  fi
  session_debug_cmd "tar-restore-powermanagement" tar -xzf "${archive}" -C /root
  [[ -f "${conf_abs}" ]]
}
